...
class ViewController: UIViewController, UITableViewDelegate {
  // MARK: - Variables
  fileprivate let mainStopwatch: Kronometre = Kronometre()
  fileprivate let lapStopwatch: Kronometre = Kronometre()
  ...
  @IBAction func playPauseTimer(_ sender: AnyObject) {
    ...
    changeButton(lapRestButton, title: "Tur", titleColor: UIColor.black)
    ...
    if !isPlay {
      ...
      isPlay = true
      changeButton(playPauseButton, title: "Durdur", titleColor: UIColor.red)
    } else {
      
      mainStopwatch.timer.invalidate()
      lapStopwatch.timer.invalidate()
      isPlay = false
      changeButton(playPauseButton, title: "Başlat", titleColor: UIColor.green)
      changeButton(lapRestButton, title: "Sıfırla", titleColor: UIColor.black)
    }
  }
  ...
  @IBAction func lapResetTimer(_ sender: AnyObject) {
    if !isPlay {
      resetMainTimer()
      resetLapTimer()
      changeButton(lapRestButton, title: "Tur", titleColor: UIColor.lightGray)
      lapRestButton.isEnabled = false
    } else {
  ...
  fileprivate func resetTimer(_ stopwatch: Kronometre, label: UILabel) {
  ...
  func updateTimer(_ stopwatch: Kronometre, label: UILabel) {
...
extension ViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    if let labelNum = cell.viewWithTag(11) as? UILabel {
      labelNum.text = "Tur \(laps.count - (indexPath as NSIndexPath).row)"
    }
